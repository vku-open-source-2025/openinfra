...
async def create_maintenance(
    ...
    return await maintenance_service.create_maintenance(
        maintenance_data, str(current_user.id)
    )
...
async def assign_maintenance(
    ...
    return await maintenance_service.assign_maintenance(
        maintenance_id, assigned_to, str(current_user.id)
    )
...
async def start_maintenance(
    ...
    return await maintenance_service.start_maintenance(
        maintenance_id, start_request, str(current_user.id)
    )
...
async def complete_maintenance(
    ...
    return await maintenance_service.complete_maintenance(
        maintenance_id, complete_request, str(current_user.id)
    )
...
async def cancel_maintenance(
    ...
    return await maintenance_service.cancel_maintenance(
        maintenance_id, cancellation_reason, str(current_user.id)
    )
...
async def upload_maintenance_photos(
    ...
    for file in files:
        # Save file
        prefix = f"{maintenance_id}/{photo_type}"
        file_url = await storage_service.upload_file(
            file=file, bucket="maintenance", prefix=prefix, filename=file.filename
        )
