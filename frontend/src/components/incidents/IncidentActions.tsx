...
import { CheckCircle, User, Wrench, X, XCircle, Archive, ShieldCheck } from "lucide-react"
...
export const IncidentActions: React.FC<IncidentActionsProps> = ({
  ...
  return (
    <div className="space-y-4">
      ...
      {incident.ai_verification_status === "to_be_verified" && canVerify && onVerify && (
        <Button
          onClick={async () => {
            setIsProcessing(true)
            try { await onVerify() } finally { setIsProcessing(false) }
          }}
          ...
          {showCloseForm ? (
            <>
              ...
              <div className="flex gap-2">
                <Button onClick={handleClose} disabled={isProcessing} className="flex-1 bg-green-600 hover:bg-green-700">
                  <Archive className="h-4 w-4 mr-2" />
                  Close Ticket
                </Button>
                <Button variant="outline" onClick={() => setShowCloseForm(false)} className="flex-1">
                  Cancel
                </Button>
              </div>
            </>
          ) : (
            <Button onClick={() => setShowCloseForm(true)} className="w-full bg-green-600 hover:bg-green-700">
              <Archive className="h-4 w-4 mr-2" />
              Close Ticket
            </Button>
          )}
      ...
      {(incident.status === "reported" || incident.status === "acknowledged") && canReject && onReject && (
