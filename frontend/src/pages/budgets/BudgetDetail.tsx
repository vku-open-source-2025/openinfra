...
const BudgetDetail: React.FC = () => {
    ...
    const createTransactionMutation = useMutation({
        mutationFn: (data: BudgetTransactionCreateRequest) =>
            budgetsApi.createTransaction(id, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["budget", id] });
                ...
                <div className="flex items-start justify-between mb-6">
                    <div>
                        ...
                        <p className="text-slate-600">
                            {budget.category} - Fiscal Year {budget.fiscal_year}
                        </p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    <div>
                        ...
                        <p className="text-2xl font-bold text-slate-900">
                            {(budget.total_amount ?? 0).toLocaleString()} VND
                        </p>
                    </div>
                    <div>
                        <h3 className="text-sm font-medium text-slate-500 mb-1">
                        ...
                        <p className="text-2xl font-bold text-red-600">
                            {(budget.spent_amount ?? 0).toLocaleString()} VND
                        </p>
                    </div>
                    <div>
                        <h3 className="text-sm font-medium text-slate-500 mb-1">
