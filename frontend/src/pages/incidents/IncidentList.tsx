...
const IncidentList: React.FC = () => {
  ...
  const { data: incidents, isLoading, error } = useQuery({
    queryKey: ["incidents", page, mainTab, verifySubTab, closedSubTab, severity],
  ...
  const filterIncidents = (data: typeof incidents) => {
    ...
    if (mainTab === "closed") {
      if (closedSubTab === "rejected") {
        ...
        filtered = filtered.filter(inc =>
          inc.resolution_type && inc.resolution_type !== "not_an_issue"
        )
      }
    }
    ...
    if (search) {
      ...
      filtered = filtered.filter(inc =>
        ...
        inc.location?.address?.toLowerCase().includes(searchLower) ||
        inc.asset?.name?.toLowerCase().includes(searchLower) ||
        inc.asset?.asset_code?.toLowerCase().includes(searchLower) ||
  ...
  const totalPages = filteredIncidents ? Math.ceil(filteredIncidents.length / limit) : 1
        ...
        <div className="flex gap-1">
          {mainTabs.map((tab) => (
            <button
              ...
              className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors ${mainTab === tab.value
        ...
        <div className="space-y-3">
          <div className="flex items-start gap-2 p-3 bg-amber-50 border border-amber-200 rounded-lg">
            ...
            <div className="text-sm text-amber-800">
              ...
              <p className="mt-1">These results are generated by AI and may contain errors. Please review each incident carefully before taking action.</p>
            </div>
          </div>
          <div className="flex gap-2">
            {[
              { value: "all" as VerifySubTab, label: "All" },
              { value: "spam_risk" as VerifySubTab, label: "🚨 Spam Risk", className: "text-red-600" },
              { value: "safe" as VerifySubTab, label: "✅ Likely Safe", className: "text-green-600" },
            ].map((tab) => (
              <button
                key={tab.value}
                onClick={() => {
                ...
                className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${verifySubTab === tab.value
                    ? "bg-slate-800 text-white"
                    : `bg-slate-100 hover:bg-slate-200 ${tab.className || "text-slate-600"}`
                  }`}
              >
                {tab.label}
              </button>
      ...
      {mainTab === "closed" && (
        <div className="flex gap-2">
          {[
            ...
            { value: "rejected" as ClosedSubTab, label: "❌ Rejected" },
            { value: "resolved" as ClosedSubTab, label: "✓ Resolved" },
          ].map((tab) => (
            <button
              key={tab.value}
              onClick={() => {
              ...
              className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${closedSubTab === tab.value
                  ? "bg-slate-800 text-white"
